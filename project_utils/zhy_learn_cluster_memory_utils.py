...
class CM(autograd.Function):
    ...
    def forward(ctx, inputs, targets, features, momentum):
        # 保存 features 和 momentum 到上下文对象 ctx
        ctx.features = features
        ctx.momentum = momentum
        # 保存 inputs 和 targets 以便在反向传播中使用
        ctx.save_for_backward(inputs, targets)
        # 计算 inputs 与 features 的内积并返回结果
    ...
    def backward(ctx, grad_outputs):
        # 从上下文对象 ctx 中恢复保存的 inputs 和 targets
        ...
        # momentum update
        # 更新 features[y] 为当前 features[y] 的加权和（使用动量 ctx.momentum）与 x（当前输入特征）的加权和
...
class CM_Hard(autograd.Function):
    ...
    def backward(ctx, grad_outputs):
        ...
        for index, features in batch_centers.items():
            ...
            # 找到距离最小的特征向量索引 median 去更新ctx.features
...
class ClusterMemory(nn.Module, ABC):
    ...
    def forward(self, inputs, targets):
        ...
        # 根据 self.use_hard 的值，选择使用 cm_hard 或 cm 函数进行特征更新
        if self.use_hard:
        ...
        # 将输出除以温度参数 self.temp 进行缩放。
        # 使用交叉熵损失函数 F.cross_entropy 计算损失
        outputs /= self.temp
