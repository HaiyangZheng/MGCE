...
class UltraFineGrainedDataset(Dataset):
    ...
    def _load_metadata(self):
        if self.train:
            self.data = pd.read_csv(
                os.path.join(self.args.UltraRoot[self.dataset_name], "anno/train.txt"),
                sep=" ",
                names=["filepath", "target"],
            )
        else:
            self.data = pd.read_csv(
                os.path.join(self.args.UltraRoot[self.dataset_name], "anno/test.txt"),
                sep=" ",
                names=["filepath", "target"],
            )
    ...
    def _check_integrity(self):
        ...
        for index, row in self.data.iterrows():
            filepath = os.path.join(self.args.UltraRoot[self.dataset_name], "images", row.filepath)
    ...
    def __getitem__(self, idx):
        ...
        path = os.path.join(self.args.UltraRoot[self.dataset_name], "images", sample.filepath)
