...
from .misc import (load_data, dump_data, mkdir_if_no_exists, Timer)
...
def fast_knns2spmat(knns, k, th_sim=0.7, use_sim=False, fill_value=None):
    ...
    # those significant enough will be chosen for graph buliding
    row, col = np.where(sims >= th_sim)
    # remove the self-loop
    idxs = np.where(row != nbrs[row, col])
    row, col = row[idxs], col[idxs]
    data = sims[row, col]
...
def build_knns(knn_prefix, feats, knn_method, k, 
               num_process=None, is_rebuild=False,
    ...
    if os.path.isfile(knn_path) and not is_rebuild and feat_create_time is not None:
        ...
        if knn_create_time <= feat_create_time: is_rebuild = True
    ...
    if not os.path.isfile(knn_path) or is_rebuild:
        ...
        with Timer('build index'):
            if knn_method == 'hnsw':
                ...
                index = knn_faiss_gpu(feats, k, index_path,
