...
def get_cub_datasets(train_transform, test_transform, federated_args,  train_classes=range(160), prop_train_labels=0.8,
...
def get_federated_cub_datasets(train_transform, test_transform, federated_args,  train_classes=range(160), prop_train_labels=0.8,
                    split_train_val=False, seed=0):
    ...
    # train_dataset_labelled = subsample_classes(deepcopy(whole_training_set), include_classes=train_classes)
    federated_train_dataset_labelled, federated_train_dataset_unlabelled = deepcopy(train_dataset_labelled), deepcopy(
    ...
    # targets = federated_train_dataset_labelled.data[:, 2]
    # for federated_args.num_clients
    if federated_args.prob_class_overlapping == 1.0:
        for i in set(federated_train_dataset_labelled.data[:, 2]):
            label_class_2_idx[i] = np.where(federated_train_dataset_labelled.data[:, 2] == i)[0]
        for i in set(federated_train_dataset_unlabelled.data[:, 2]):
            unlabel_class_2_idx[i] = np.where(federated_train_dataset_unlabelled.data[:, 2] == i)[0]
    ...
    for index_client in range(federated_args.num_clients):
        federated_train_dataset_labelled[index_client] = subsample_dataset(deepcopy(train_dataset_labelled),
                                                                           np.concatenate(federated_train_idx_labelled[
                                                                                              index_client], axis=0))
