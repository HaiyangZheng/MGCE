...
class CustomCub2011(Dataset):
    ...
    def _load_metadata(self):
        ...
        self.data['filepath'] = base_path + os.sep + self.data['filepath']
        ...
        ## added by hyzheng for targets
        self.targets = (self.data['target'] - 1).tolist()
...
# def subsample_dataset(dataset, idxs):
#     # 直接使用idxs，不创建mask
#     dataset.data = dataset.data.iloc[idxs].reset_index(drop=True)
#     dataset.uq_idxs = dataset.uq_idxs[idxs]
    
#     # 从更新后的data重新生成targets
#     dataset.targets = (dataset.data['target'] - 1).tolist()
...
def get_cub_datasets(train_transform, test_transform, train_classes=range(160), prop_train_labels=0.8,
    ...
    whole_training_set = CustomCub2011(root=args.cub_root, transform=train_transform, train=True, download=download)
    ...
    test_dataset = CustomCub2011(root=args.cub_root, transform=test_transform, train=False)
...
if __name__ == '__main__':
    ...
    args.cub_root = '/leonardo_work/IscrC_DiffGRE/Datasets/CUB'

    x = get_cub_datasets(None, None, split_train_val=False,
                         train_classes=range(100), prop_train_labels=0.5, args=args)
    ...
    for i in range(10):
        print(x['train_labelled'].data.iloc[i].filepath, x['train_labelled'].targets[i], x['train_labelled'].uq_idxs[i])
