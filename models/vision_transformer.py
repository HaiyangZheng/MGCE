...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=12,
                 ...
                 drop_path_rate=0., norm_layer=nn.LayerNorm, num_prompts=0, **kwargs):
        super().__init__()
        ...
        if num_prompts!=0:
            ...
            self.vpt_drop = nn.ModuleList([nn.Dropout(p=0) for d in range(depth)])
...
def vit_base(patch_size=16, **kwargs):
    model = VisionTransformer(
        ...
        qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), num_prompts=kwargs['num_prt'], **kwargs)
    return model
