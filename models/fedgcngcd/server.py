...
class Server(ServerModule):
    ...
    def test_by_kmeans(self, curr_rnd):
        if curr_rnd is not None:
            pass
        if self.curr_rnd % self.args.test_interval == 0 and self.curr_rnd > 0:
    ...
    def test_kmeans(self, backbone, test_loader,
        ...
        print(f'Using contrastive feature Fitting K-Means... k={hyperparameter_K}')
        kmeans = KMeans(n_clusters=hyperparameter_K, random_state=0).fit(all_feats)
        preds = kmeans.labels_

        print('Using contrastive feature Done')
