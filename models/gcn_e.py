...
from models.utils import GraphConv, MeanAggregator
...
class GCN_E(nn.Module):
    def __init__(self, feature_dim, nhid, nclass, dropout=0):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.PReLU(nhid_half), nn.Linear(nhid_half, self.nclass)
        )
        if nclass == 1:
...
def gcn_e(feature_dim, nhid=512, nclass=1, dropout=0., **kwargs):
    model = GCN_E(
        feature_dim=feature_dim, nhid=nhid, 
        nclass=nclass, dropout=dropout
    )
    return model
