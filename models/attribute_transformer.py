...
from models.meta_graph import GraphConvolution
...
class vit_backbone(nn.Module):
    def __init__(self, vit_backbone_model, grad_from_block=11):
        ...
        self.prt_size = getattr(vit_backbone_model, 'num_prompts', None)
        if self.prt_size:
...
class AttributeTransformer13(nn.Module):
    def __init__(self, vit_backbone_model, num_classes=0, 
                 num_attribute=28, attribute_feat_channal=8, grad_from_block=11):
        super().__init__()
        self.feature_extractor = vit_backbone(vit_backbone_model=vit_backbone_model,
                                              grad_from_block=grad_from_block)
        for m in self.feature_extractor.parameters():
...
def at13_base(pretrain_path, num_attribute=28, attribute_feat_channal=8, 
              grad_from_block=11, pre_train=True, num_prt=0, use_simgcd=False):
    weight = torch.load(pretrain_path, map_location='cpu')
    if use_simgcd:
        backbone = torch.hub.load('facebookresearch/dino:main', 'dino_vitb16')
        projector = DINOHead(in_dim=768, out_dim=200, nlayers=3)
        student = nn.Sequential(backbone, projector)
    ...
    if pre_train:
        msg = student.load_state_dict(weight, strict=False)
        if flag: student = student[0]
    model = AttributeTransformer13(student, num_attribute, attribute_feat_channal, 
                                   grad_from_block)
    return model
